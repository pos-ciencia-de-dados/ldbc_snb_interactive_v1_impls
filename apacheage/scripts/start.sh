...
if [ -n "${AGE_CONVERTED_CSV_DIR-}" ]; then
    if [ ! -d "${AGE_CONVERTED_CSV_DIR}" ]; then
        echo "Directory ${AGE_CONVERTED_CSV_DIR} does not exist."
        exit 1
    fi
    export MOUNT_CSV_DIR="--volume=${AGE_CONVERTED_CSV_DIR}:/data:z"
else
    export AGE_CONVERTED_CSV_DIR="<unspecified>"
...
echo "AGE_CONVERTED_CSV_DIR (on the host machine):"
echo "  ${AGE_CONVERTED_CSV_DIR}"
echo "==============================================================================="
docker pull ${POSTGRES_DOCKER_IMAGE}:${POSTGRES_VERSION}
