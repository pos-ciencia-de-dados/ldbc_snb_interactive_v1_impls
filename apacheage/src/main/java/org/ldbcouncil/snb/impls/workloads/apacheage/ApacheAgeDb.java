...
import org.ldbcouncil.snb.impls.workloads.apacheage.operationhandlers.*;
...
public abstract class ApacheAgeDb extends BaseDb<PostgresQueryStore> {
    ...
    public static class Query2 extends PostgresListOperationHandler<LdbcQuery2, LdbcQuery2Result> {
        ...
        public PreparedStatement setParametersInPreparedStatement(Operation operation, String queryString, Connection conn) throws SQLException {
    		...
    		builder.add("personId", (Long)parameterMap.get("personId"));
    		builder.add("maxDate", ((Date)parameterMap.get("maxDate")).getTime());

    		Agtype obj = AgtypeFactory.create(builder.build());
        ...
        public LdbcQuery2Result convertSingleResult(ResultSet result) throws SQLException {
            return new LdbcQuery2Result(
                    ...
                    result.getLong(4),
                    result.getString(5),
                    Long.parseLong(result.getObject(6, Agtype.class).getValue()));
        }
